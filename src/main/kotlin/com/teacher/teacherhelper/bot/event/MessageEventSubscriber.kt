...
class MessageEventSubscriber(
    ...
    override val eventType = MessageEvent::class

    // 受管作用域（随 Bean 生命周期取消）
    private val job = SupervisorJob()
    private val scope = CoroutineScope(job + Dispatchers.Default)

    override fun subscribe(): suspend MessageEvent.(MessageEvent) -> Unit = handler@{ event ->
        if (event.message.source.kind != MessageSourceKind.FRIEND) return@handler
